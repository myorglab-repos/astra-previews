...
export async function createViewer(stage) {
  ...
  let gltf;
  try { gltf = await loader.loadAsync('./hero-plates-armor-m.web.glb'); }
  catch { gltf = await loader.loadAsync('./hero-plates-armor-m.glb'); }
  ...
  const seamMaterials = [];
  gltf.scene.traverse(o => { if (o.isMesh && o.material?.name.startsWith('seam_')) { o.material.toneMapped=false; o.material.side=THREE.DoubleSide; seamMaterials.push({material:o.material,phase:(Number(o.material.name.slice(-2))+.5)/16}); } });
  const baseYaw = -.22, basePitch = .105;
  rig.rotation.set(basePitch, baseYaw, 0);
  function lightAt(t, envelope = 0) {
    for (const s of seamMaterials) s.material.emissiveIntensity = .09 + 5.2*(.5+.5*Math.cos(2*Math.PI*(t/3-s.phase)))**4 + 2.2*envelope;
  }
  ...
  function draw(now) {
    ...
    rig.rotation.y += (baseYaw+pointer.x*.055-rig.rotation.y)*follow;
    rig.rotation.x += (basePitch+pointer.y*.035-rig.rotation.x)*follow;
    const envelope = beatAge < 1.3 ? Math.sin(Math.PI*beatAge/1.3)**2 : 0;
    rig.scale.setScalar(1 + .038*envelope);
    lightAt(mixer.time, envelope);
  ...
  function pause() {
    ...
    pointer.set(0, 0); rig.rotation.set(basePitch, baseYaw, 0); rig.scale.setScalar(1); beatAge = 10;
  }
  function play() {
  ...
  window.heroReview = { gltf, mixer, rig, renderer, scene, camera, seamMaterials, lightAt, baseYaw, basePitch };
