...
for p in ['unit_verification.json','integration_verification.json','model_verification.json','media_verification.json']:assert read(p)['status']=='PASS'
...
for n in ['EXECUTIVE_REVIEW.md','READY_FOR_OLA.md']:
    ...
    assert all(t in body for t in ['Plain-language glossary','Track A does not prove strike impulse','B-06','C-01','synthetic'])
...
(R/'packet_verification.json').write_text(json.dumps(dict(status='PASS_WITH_ENGINEERING_RESIDUALS',manifest_artifacts=len(manifest),local_links_checked=len(links),missing_links=missing,baseline_hashes=11,inherited_scenes=5,received_frames=144,film_frames=144,key_stills=8,source='synthetic translated-still clip; simulated presence and faults',independent_ola_acceptance='PENDING',open_items=['Real-camera/proximity bench','Lead mapping / sign / idle / watchdog freeze','B-06/C-01 Critical hardware and propulsion','SAF-02/P-05 physical stop / limp','Loaded clearance and durability'],scope='Track A does not prove strike impulse.'),indent=2))
print('PACKET_VERIFIED',len(manifest),'artifacts;',len(links),'local links')
